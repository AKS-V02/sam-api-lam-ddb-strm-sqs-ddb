        ...
        try {
            ...
            if(queryParameter!=null && queryParameter.keySet().contains(primaryKey)){
                if(queryParameter.keySet().contains(sortKey)){
                    Output responseInput = mapper.load(Output.class, queryParameter.get(primaryKey), 
                    ...
                    DynamoDBQueryExpression<Output> quiryExpression = new DynamoDBQueryExpression<Output>().withLimit(3)
                    .withKeyConditionExpression(primaryKey+" = :val1 and "+lsiKey+" = :val2")
                    .withIndexName(lsiName)
                    .withExpressionAttributeValues(eav);
                    if(queryParameter.containsKey("lastUserId") && queryParameter.containsKey("lastFullName")){
                        Map<String, AttributeValue> exclusiveStartKey = new HashMap<>();
                        exclusiveStartKey.put(sortKey,new AttributeValue().withS(queryParameter.get("lastUserId")));
                        exclusiveStartKey.put(primaryKey,new AttributeValue().withS(queryParameter.get("lastFullName")));
                        quiryExpression.setExclusiveStartKey(exclusiveStartKey);
                    }
                    QueryResultPage<Output> quiryResponse = mapper.queryPage(Output.class, quiryExpression);
                ...
                if(queryParameter!=null && queryParameter.containsKey("lastUserId") && queryParameter.containsKey("lastFullName")){
                    Map<String, AttributeValue> exclusiveStartKey = new HashMap<>();
                    exclusiveStartKey.put(sortKey,new AttributeValue().withS(queryParameter.get("lastUserId")));
                    exclusiveStartKey.put(primaryKey,new AttributeValue().withS(queryParameter.get("lastFullName")));
                    scanExpression.setExclusiveStartKey(exclusiveStartKey);
                }
                ScanResultPage<Output> scanResponse = mapper.scanPage(Output.class, scanExpression);
